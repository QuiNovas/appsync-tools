...
from .exceptions import MultipleRoutesFoundExcepion, NoRouteFoundException, RouteAlreadyExistsException
...
class Router:
    ...
    def handle_route(self, event: Dict) -> Any:
        ...
        if len(routes_to_call) == 0:
            if self.default_route is not None:
                res = self.default_route.callable(event)
            else:
                raise NoRouteFoundException(f"No matching routes for {path}")
        else:
            res = []
            for route in routes_to_call:
                res.append(route.callable(event))

            if self.allow_multiple_routes is True:
                res = res[0]
